...
export default function App() {
  ...
  const setupWallets = async () => {
    ...
    try {
      addLog("stellar", "Generating keypairs for manager + all agents...");
      const mgr        = StellarSdk.Keypair.random();
      const kps        = {};
      const current    = loadAgents();
      for (const a of current) kps[a.id] = StellarSdk.Keypair.random();

      addLog("stellar", `Funding ${current.length + 1} wallets via Stellar Friendbot...`);
      await Promise.all([
        friendbot(mgr.publicKey()),
        ...Object.values(kps).map(kp => friendbot(kp.publicKey())),
      ]);

      const bal = await getBalance(mgr.publicKey());
      setManagerKP(mgr); setAgentKPs(kps); setManagerBal(bal);
      setWalletStatus("ready");
      addLog("stellar", `Manager: ${bal.toFixed(4)} XLM — ${current.length} agent wallets funded.`);
  ...
  const run = async () => {
    ...
    try {
      ...
      // Manager plans dynamically based on all registered agents
      const agentList = agents.map(a =>
        `- id:"${a.id}" name:"${a.label}" skill:"${a.desc}" price:${a.price}XLM`
      ).join("\n");

      const raw = await callClaude(
        `You are a Manager Agent coordinating a marketplace of specialist AI agents.
Assign a specific instruction to EVERY available agent listed.
Agents execute sequentially — each receives all previous agents' outputs.
Return ONLY valid JSON, no markdown:
{ "assignments": [ { "agentId": "...", "instruction": "..." } ] }`,
        `Available agents:\n${agentList}\n\nTask: "${task}"\n\nCreate a tailored instruction for every agent. Build on previous agents' work where relevant.`
      );
      ...
      for (const { agentId, instruction } of assignments) {
        ...
        addLog("tx", `Signing Stellar payment to ${abbr(agentKPs[agentId].publicKey())}...`, agentId);
        const hash = await stellarPay(managerKP, agentKPs[agentId].publicKey(), cfg.price);
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        input:focus { outline:none!important; box-shadow:0 0 0 1px #4ADE8066!important }
